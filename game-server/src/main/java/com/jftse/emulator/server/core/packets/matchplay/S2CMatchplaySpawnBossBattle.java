...
public class S2CMatchplaySpawnBossBattle extends Packet {
    public S2CMatchplaySpawnBossBattle(GuardianBase boss, GuardianBase leftMonster, GuardianBase rightMonster) {
        ...
        this.write(boss.getId().byteValue()); // Boss index (BossGuardianInfo.set + 3)
        this.write(leftMonster.getId().byteValue()); // Left Sideboss index (GuardianInfo.set)
        this.write(rightMonster.getId().byteValue()); // Right Sideboss index (GuardianInfo.set)
    }
}
