...
@Scope("singleton")
@Getter
public class StaTableService {
    private List<StaValue> staValues;

    @PostConstruct
    public void init() throws IOException {
        InputStream inputStream = ResourceUtil.getResource("res/StaTable.json");
        try {
            Reader reader = new InputStreamReader(inputStream);
            Type collectionType = new TypeToken<List<StaValue>>() {}.getType();
            Gson gson = new Gson();
            staValues = gson.fromJson(reader, collectionType);
        } finally {
            inputStream.close();
        }
    }
