...
@Scope("singleton")
@Getter
public class StrTableService {
    private List<StrValue> strValues;

    @PostConstruct
    public void init() throws IOException {
        InputStream inputStream = ResourceUtil.getResource("res/StrTable.json");
        try {
            Reader reader = new InputStreamReader(inputStream);
            Type collectionType = new TypeToken<List<StrValue>>() {}.getType();
            Gson gson = new Gson();
            strValues = gson.fromJson(reader, collectionType);
        } finally {
            inputStream.close();
        }
    }
